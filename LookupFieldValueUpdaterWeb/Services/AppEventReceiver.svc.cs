...
namespace LookupFieldValueUpdaterWeb.Services
{
    public class AppEventReceiver : IRemoteEventService
    {
        ...
        /// </summary>
        /// <param name="properties"></param>
        private void HandleItemAddedAsync(SPRemoteEventProperties properties)
        {
            using (ClientContext clientContext =
                TokenHelper.CreateRemoteEventReceiverClientContext(properties))
            {
                if (clientContext != null)// && EventFiringEnabled(clientContext, properties, "2"))
                {
                    new RemoteEventReceiverManager().ItemAddedToListEventHandlerAsync(clientContext, properties.ItemEventProperties.ListId, properties.ItemEventProperties.ListItemId);
                }
            }
        }
